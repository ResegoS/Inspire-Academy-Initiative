        ...
        if($user_email == 0)
        {
          if($password==$confirm_password)
          {
            if($email==$confirm_email)
            {
                ...
                //Storing data
                $sql=$conn->prepare("INSERT INTO registration(user_id,first_name,last_name,user_type,password,email,contact,address,education)
                VALUES ('$user_id','$first_name','$last_name','$user_type','$password','$email','$contact','$address','$school')");
                $sql->execute();
                ...
                {
                  $_SESSION['success'] = "You Have Successfully Registered";
                  header('location:student-index-php.php');
                }
              }
